...
import { Game, GameFN, Scoreboard, Score, Karaoke_Solo, KaraokeCurrentSong, KaraokeSongSegment, KaraokeSongLyrics, KDFN, Karaoke_Duett } from '@/types';
import AudioRecorder from '../AudioRecorder';
import { getUserAvatar } from '@/lib/api';

interface KaraokeDuettProps {
  isHost: boolean;
  GameData: Game;
  KDFN: KDFN;
  GameFN: GameFN;
...
export default function KaraokeDuett({ isHost, GameData, KDFN, GameFN, currentSong, autoPlay, RTP, playFinal }: KaraokeDuettProps) {
  const { user } = useUser();
  ...
  const { RecordCallBack, setAutoPlayState, startPlayback, resetRTP, startRound, openVote, voteToPlayer, setPlayFinal, reqPlayFinal } = KDFN
  ...
  useEffect(() => {
    console.log(`playFinal: ${playFinal} | getFinalUrl: ${getFinalUrl()}`)
    if (!playFinal) return;
    if (!getFinalUrl() || getFinalUrl() == "") return;
    const finalSong = document.getElementById(`final-audio`) as HTMLAudioElement;
    if (!finalSong) return;
    finalSong.volume = volume
    finalSong.play();
  ...
  const getFinalUrl = () => {
    if (!GameData || !GameData.currentGameModeData) return "";
    const cGameData = (GameData.currentGameModeData as Karaoke_Duett);
    if (!cGameData.finalOutput) return "";
    return `https://gameapi.katroland.hu/karaoke/output/${cGameData.finalOutput}`;
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className={`w-10 h-10 bg-gradient-to-br rounded-full flex items-center justify-center text-white font-bold shadow-lg ${isCurrentUser
              ...
              <img src={getUserAvatar(player.playerId)} alt="" className="w-full h-full rounded-full" onError={(e) => {
        ...
        {isHost ? (
          <div className="mb-6 p-4 bg-white/5 rounded-xl border border-white/10">
            ...
            <div className='w-full flex justify-center items-center mt-4'>
              <button
                onClick={() => reqPlayFinal()}
                className="px-3 py-1 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 rounded-full text-xs text-white font-medium transition-all duration-200 hover:scale-105 active:scale-95 h-12 w-48 flex items-center justify-center font-bold font-medium"
              >
                Request Play Final Mix
              </button>
            </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/20">
          <div className="mb-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <audio id='final-audio' src={getFinalUrl()} hidden></audio>
